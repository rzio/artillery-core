...
function phaser(phaseSpecs, scenarioEvents) {
  ...
  let tasks = _.map(phaseSpecs, function(spec, i) {
    ...
    } else if (spec.concurrentScenarios){
      return createConcurrent(spec, ee, scenarioEvents);
    } else {
...
function createConcurrent(spec, ee, scenarioEvents) {
   let finished = false; 
   scenarioEvents.on('scenarioCompleted',function(){
       ee.emit('arrival');
   }) 
  ...
  const task = function(callback) {
    ee.emit('phaseStarted', spec);
    const duration = spec.duration * 1000;
    for(let i=0; i< spec.concurrentScenarios; i++) {
        ee.emit('arrival');
    }
    setTimeout(function() {
        ee.emit('phaseCompleted', spec);
