...
function run(script, ee, options) {
  ...
  let phaserScenarioEvents = new EventEmitter();
  let phaser = createPhaser(script.config.phases, phaserScenarioEvents);
  phaser.on('arrival', function() {
    runScenario(script, intermediate, aggregate, phaserScenarioEvents);
  });
...
function runScenario(script, intermediate, aggregate, phaserScenarioEvents) {
  ...
  if (!compiledScenarios) {
    ...
    scenarioEvents.on('started', function() {
      pendingScenarios++;
      phaserScenarioEvents.emit('scenarioStarted');
    });
  ...
  compiledScenarios[i](scenarioContext, function(err, context) {
    ...
    phaserScenarioEvents.emit('scenarioCompleted');
  });
}
